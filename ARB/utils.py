...
async def zone_checker(zone):
    if zone == "WizardCity/WC_Streets/WC_Drains/Z00_ClubHouse":
        ...
        print("This dungeon is not farmable by ARB.")
    return True


async def house_floor1(*clients):
    for client in clients:
        await asyncio.sleep(1)
        await asyncio.gather(*[client.send_key(Keycode.W, 3)])
        await asyncio.sleep(1)
        await asyncio.gather(*[go_through_dialog(client) for client in clients])
        await asyncio.sleep(1)


async def initiate_combat(*clients):
    print("yes")
